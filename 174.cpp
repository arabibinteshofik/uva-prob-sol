...
public:
    ...
    bool cond(char *&p, Memory MY, Memory YOUR)
    {
        ...
        if (q != NULL)
            p = q, kind = "EMPTY";
        bool ret = equal == "=" ? addr->getMemory(read) == Memory::getAction(kind) : addr->getMemory(read) != Memory::getAction(kind);
        // cout << "ADDR: " << addr->getMemory(read) << ", ACTION: " << Memory::getAction(kind) << endl;
        // cout << read << ", " << kind << endl;
        /* <op> <condition> */
        q = match(p, "AND");
        if (q != NULL)
            p = q, ret = ret & cond(p, MY, YOUR);
        q = match(p, "OR");
        if (q != NULL)
            p = q, ret = ret | cond(p, MY, YOUR);
    ...
    ACTION eval(char *&p, Memory a, Memory b)
    { // read only
        // printf("eval %s\n", p);
